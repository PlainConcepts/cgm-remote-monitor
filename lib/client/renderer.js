...
function init (client, d3) {
  ...
  renderer.addFocusCircles = function addFocusCircles ( ) {
    ...
    function prepareFocusCircles(sel) {
      ...
      sel.attr('cx', function (d) {
        ...
        .attr('fill', function (d) {
          return d.type === 'forecast' ? d.color : d.color;
        })
    ...
    function focusCircleTooltip (d) {
      ...
      var targetBounds = d3.event.target.getBoundingClientRect();

      client.tooltip.transition().duration(TOOLTIP_TRANS_MS).style('opacity', 1);
      client.tooltip.html('<strong>' + bgType() + translate(' BG')+ ':</strong> <span class="number">' + client.sbx.scaleEntry(d) + '</span>' +
        ...
        .style('right', 'auto')
        .style('left', (targetBounds.left - 75 + 4) + 'px')
        .style('top', (targetBounds.top - 125 + 4) + 'px');
    }
