...
window.Settings = {
  ...
  topChart: {
    left: 50,
    right: 0,
    width: function() { return window.Settings.canvas.width - window.Settings.topChart.left - window.Settings.topChart.right }
  },
  page: {
    width: function() { return document.documentElement.clientWidth || screen.width }
  }
}
...
// clears the current user brush and resets to the current real time data
  ...
  function updateClock() {
    ...
    /** Disabled night mode code, it doesn't have sense with the actual theme
  ...
  function updateClockDisplay() {
    ...
    client.now = Date.now();
    $('#currentTime').text(formatTime(new Date(client.now), false));
  }
  ...
  function checkTimeAgoAlarm(ago) {
    ...
    if (isStale(ago) && notAcked(alarm)) {
      ...
      var notify = {
        title: 'Last data received ' + [ago.value, ago.label].join(' ')
        , level: level === 'urgent' ? 2 : 1
      };
  ...
  function updateTimeAgo() {
    ...
    function updateTimeAgoPill() {
      if (retroMode || !ago.value) {
        ...
        lastEntry.removeClass('lastentry-alert');
      } else {
        lastEntry.find('em').show().text(ago.value);
        if(ago.value > 5) {
          lastEntry.addClass('lastentry-alert');
        }else {
          lastEntry.removeClass('lastentry-alert');
        }
      }
  ...
  $('#testAlarms').click(function(event) {
    d3.selectAll('.audio.alarms audio').each(function () {
      var audio = this;
      playAlarm(audio);
      $('#testAlarms i.icon-sound').removeClass('icon-sound-mute');
      setTimeout(function() {
        $('#testAlarms i.icon-sound').addClass('icon-sound-mute');
        audio.pause();
      }, 4000);
    });
    event.preventDefault();
